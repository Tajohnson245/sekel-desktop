 ...
 * no intelligence, no exam readiness yet) are filtered out so we never
 * spotlight an empty UI.
 */
export function useVisibleTourStepIds(): string[] {
    ...
    return useMemo(
        () => {
            const hasExamReadiness =
                !!examProfile?.exam_date &&
                !!intelligence?.hasClassifications &&
                (intelligence?.systemBreakdown?.length ?? 0) > 0;
            const hasSystemCoverage =
                (activePlan?.plan?.snapshot?.systemCoverage?.length ?? 0) > 0;
            return visibleStepIdsFor({
