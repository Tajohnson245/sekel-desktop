...
import type { DeckState } from '../../main/import/importOptionsLogic';
import type {
    ImportSummary,
    ImportSummaryDeck,
...
export default function ImportOptionsModal({
    ...
    // Group decks by root name for tree rendering
    ...
    const handleConfirm = useCallback(async () => {
        ...
        try {
            const payload = buildPayload(summary, deckStates, mediaMap, mediaFilePaths, tempDir);
            await onConfirm(payload);
        } finally {
            setIsSubmitting(false);
        }
    }, [summary, deckStates, mediaMap, mediaFilePaths, tempDir, onConfirm]);
        ...
        return (
            <div key={deck.ankiDeckId}>
                ...
                >
                    ...
                    >
                        {depth === 0 ? deck.nameComponents[0] : deck.nameComponents.slice(1).join('::')}
                        {deck.hasConflict && (
                ...
                {/* Children (expanded) */}
                {depth === 0 && hasChildren && state.expanded &&
