...
// Tracked so update events from the main process can be pushed to the
// renderer regardless of which createWindow call mounted the window.
let mainWindowRef: BrowserWindow | null = null;

// publisher-github targets Tajohnson245/sekel (see apps/desktop/forge.config.js).
// Auto-update notes live on releases of that repo.
...
app.whenReady().then(() => {
    ...
    // Check for updates only in production (packaged app). notifyUser is off
    // because the renderer shows a custom modal — see UpdateAvailableModal.
    // The update-downloaded handler below pushes the GitHub Release notes
    // (auto-generated by the publish-notes job in .github/workflows/release.yml)
    // into the renderer for display.
    if (app.isPackaged) {
        updateElectronApp({
            updateSource: {
